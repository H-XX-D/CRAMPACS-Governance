# CRAMPACS Program Outline and End-to-End SOP

**CRAMPACS:** Coordinate-Resolved Anomaly Meta-analysis with Pre-specified Anomaly Cluster Statistics  
**Version:** 0.1  
...
CRAMPACS is a coordinate-resolved meta-analytic framework for testing whether weak anomalies from heterogeneous experimental catalogs cluster at pre-specified parameter values more often than expected under controlled null models.

The method does not treat weak anomalies as discoveries. It treats cross-catalog recurrence at the same coordinate as a testable statistical object. The result of a CRAMPACS study is an anomaly-prioritization and coherence assessment, not a physics claim by itself.
...
CRAMPACS fills the evidentiary layer between isolated unresolved anomalies and confirmed discovery. It is designed to answer whether many weak or sub-threshold observations show structured recurrence across catalogs, while preserving the discovery standards of the underlying fields.

CRAMPACS may support statements such as:
...
CRAMPACS may not support statements such as:
...
CRAMPACS should be built as a physics-adapted evidence synthesis standard. It borrows discipline from systematic review, statistical reporting, metrology, repository governance, and AI risk management, but adapts each to anomaly catalogs.
...
These are not copied wholesale. CRAMPACS uses them as alignment targets.
...
2. **Exploration and confirmation are different products.** An exploratory anomaly atlas may generate candidate coordinates. A confirmatory CRAMPACS run may only test coordinates locked before scoring.
...
4. **Independence is graded, not assumed.** Multiple rows from the same detector, collaboration, public dataset, calibration pipeline, simulation stack, or theory-derived search band may share evidence. CRAMPACS must model this dependence.
...
6. **The result is a ranking, not a verdict.** CRAMPACS can identify coordinates worth attention. It cannot replace direct replication, targeted searches, or field-specific discovery thresholds.
...
CRAMPACS must exclude or quarantine:
...
- Rows whose coordinate value was chosen after seeing the CRAMPACS target result.
- Duplicates of the same analysis that cannot be separated from the original evidence source.

### 5.3 Confirmatory Boundary

A confirmatory CRAMPACS result requires:
...
The adapter layer maps domain-specific catalogs into the common CRAMPACS contract.
...
Every CRAMPACS study must assign named owners. A single person may hold multiple roles in a pilot, but role separation is required before external release.
...
CRAMPACS agents may be human, software, or AI-assisted workers. Agents do not decide scientific truth. They perform bounded tasks under a documented contract.
...
2. Create a study identifier using `CRAMPACS_<domain>_<YYYY-MM-DD>`.
...
1. Draft a protocol using PRISMA-P structure adapted to CRAMPACS.
...
- `CRAMPACS_PROTOCOL_LOCK_<YYYY-MM-DD>.md`
- `protocol_hash.txt`
- `amendment_log.csv`
...
1. Title identifying the study as CRAMPACS and whether it is exploratory, confirmatory, or prospective.
...
- Tier 1: Locked retrospective CRAMPACS signal. Candidate and scoring were pre-specified before scoring existing data.
- Tier 2: Independently reproduced CRAMPACS signal. Another team re-ran the same package and obtained equivalent results.
...
No CRAMPACS report may quote a local p-value, sigma-equivalent, or posterior odds as the headline result unless the global correction is presented next to it.
...
CRAMPACS should not claim formal accreditation unless an external accredited body grants it. The correct near-term target is "accreditation-ready" or "standard-aligned."
...
Use ISO/IEC 17025 principles where CRAMPACS behaves like a testing activity:
...
CRAMPACS is not a calibration laboratory by default. The standard is useful because it forces method competence and traceability.
...
- Documented training on CRAMPACS extraction and bias rules.
...
The first CRAMPACS pilot should be narrow. A good pilot is not "all anomalies in physics." It should use one coordinate family and a bounded catalog.
...
crampacs/<study_id>/
  00_charter/
    ...
    agent_registry.csv
  01_protocol/
    CRAMPACS_PROTOCOL_LOCK_<date>.md
    protocol_hash.txt
    amendment_log.csv
  02_sources/
    source_search_log.csv
...
> In a locked CRAMPACS analysis, candidate coordinate X showed cross-catalog anomaly clustering above the registered null expectation. The result remained stable under the registered sensitivity tests and is best interpreted as a prioritization signal for prospective testing.

Approved exploratory language:

> The exploratory atlas identified a possible recurrence near coordinate X. Because this coordinate was not locked before atlas inspection, the result is hypothesis-generating only and requires a new confirmatory CRAMPACS protocol.

Required limitation language:

> CRAMPACS does not establish a discovery claim. It tests whether structured recurrence exists under a specified evidence, dependence, and null-model contract. Direct experimental confirmation remains necessary.

## 18. Immediate Next Actions

1. Create the first CRAMPACS pilot workspace.
...
The program should start narrow and severe. A small, well-controlled CRAMPACS study is more valuable than a broad anomaly story that cannot survive independence, missing-evidence, and look-elsewhere review.
