# CRAMPACS Standards and Practices Policy

**Policy ID:** CRAMPACS-SPP-001  
**Version:** 0.1  
**Date:** 2026-05-15 PDT  
...
This policy defines the standards, practices, quality controls, reporting obligations, and governance requirements for CRAMPACS studies.

CRAMPACS exists to test whether weak anomalies, residuals, nulls, exclusions, and near-misses show statistically unusual recurrence at pre-specified coordinates. Its credibility depends on stricter-than-normal discipline around pre-specification, source completeness, independence, null models, and conservative reporting.

## 2. Authority

This policy applies to all CRAMPACS work products:
...
No CRAMPACS output may be promoted externally unless it satisfies this policy or documents a formal deviation.

## 3. Governing Standard Stack

CRAMPACS uses the following standard stack as alignment targets:

| Area | Alignment target | CRAMPACS use |
|---|---|---|
...
Every confirmatory CRAMPACS study must lock:
...
CRAMPACS recognizes three work modes:
...
No CRAMPACS report may headline an uncorrected local result.
...
Every externally shared CRAMPACS result must include:
...
CRAMPACS may claim "accreditation-ready" only when the following are present:
...
CRAMPACS may not claim formal accreditation without external accreditation.
...
> CRAMPACS is an anomaly synthesis and prioritization method. It does not replace direct experimental confirmation or field-specific discovery thresholds.
...
- At least every six months while CRAMPACS is active.
