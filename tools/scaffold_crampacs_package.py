#!/usr/bin/env python3
"""Create a full CRAMPACS evidence-package binder scaffold."""
...
TEMPLATE_COPIES = [
    ...
    ("templates/CRAMPACS_PROTOCOL_TEMPLATE.md", "01_protocol_lock/protocol.md"),
...
def load_domain(slug: str) -> dict:
    domains = json.loads((ROOT / "tools" / "crampacs_domains.json").read_text(encoding="utf-8"))
...
**Domain:** {domain["label"]}  
**Created:** {created}  
**Assurance level:** full CRAMPACS package scaffold

This package is not release-ready when created. It is a controlled binder scaffold.
Run `python tools/crampacs_sidecar.py {out_dir} --level full` from the repository root during assembly and before release.
...
CRAMPACS does not establish causality, compliance, safety, efficacy, exploitability, fraud, or physical discovery by itself.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scaffold a full CRAMPACS evidence-package binder.")
